...
public class NRandGenerator<V, E> {
	...
	/**
	 * This method generates grid2D graph. Generator from JUNG libray is used in
	 * this method.
	 *
	 ...
	 * @return grid2D graph
	 */
	public static <V, E> Graph<V, E> generateGridGraph(Factory<Graph<V, E>> graphFactory, Factory<V> vertexFactory,
			Factory<E> edgeFactory, int width, int height, boolean torus) {
	 ...
	 */
	public static <V, E> Graph<V, E> generateRegularRing(Factory<Graph<V, E>> graphFactory, Factory<V> vertexFactory,
			Factory<E> edgeFactory, int n, int k) {
		...
		for (int j = 0; j < k / 2; j++) {
			for (int i = 0; i < n; i++)
				G.addEdge(edgeFactory.create(), vertices.get(i), vertices.get((i + 1 + j) % n));
		}
