...
public class Export {
  ...
  public static<V,E> void writeNet(Graph<V,E> G, String fname)
  {
        ...
        for(V v : G.getVertices())
           { output.println(i +" \""+v.toString()+"\""); posv.put(v, i); i++; }
        ...
        for(V v : G.getVertices())
        {
            ...
            for(V nv : G.getNeighbors(v))
            {
                ...
                if(!done.contains(p1) &&!done.contains(p2))
                    if(first)
                    { output.print(posv.get(v)+" ");  output.print(posv.get(nv)+" "); done.add(p1); done.add(p2); first=false; }
                    else
                    {  output.print(posv.get(nv)+" "); done.add(p1); done.add(p2); }
            }
 ...
 public static<V,E> void writeDNet(Graph<V,E> G, String fname)
  {
        ...
        for(V v : G.getVertices())
        {
            ...
            for(V nv : G.getSuccessors(v))
            {
                ...
                if(!done.contains(p))
                    if(first)
                    { output.print(posv.get(v)+" ");  output.print(posv.get(nv)+" "); done.add(p); first=false; }
                    else
                    {  output.print(posv.get(nv)+" "); done.add(p); }
            }
