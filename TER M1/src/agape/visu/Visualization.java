...
public class Visualization {
	...
	protected static final class ColoredSetsTransformer<V> implements Transformer<V, Paint> {
		...
		/**
		 * Transform a vertex n by returning the Paint element associated to it. It
		 * takes into account the group of vertices the vertex n belongs to.
		 *
	...
	/**
	 * @author J.-F. Lalande This class is used to transform each set of vertices
	 *         into a random color. It uses a Set of Set of vertices. Each set of
	 *         vertices should take a color. As colors are choosen randomly, two
	 *         groups may have the same or colors that are similar.
	 * @param <V>
	 */
	protected static final class ColoredSetsTransformerSet<V> implements Transformer<V, Paint> {
		...
		/**
		 * Constructs the transformers and take a set of set of vertices. Each set of
		 * vertices should take a color.
		 *
	...
	public static <V, E> void showGraph(Graph<V, E> G) {
		Visualization.showGraph(G, new HashSet<V>());
	}
	...
	public static <V, E> void showGraph(Graph<V, E> G, Layout<V, E> l) {
		Visualization.showGraph(G, new HashSet<V>(), null, l);
	}

	/**
	 * This method allows a fast visualization of a graph. If the selectednodes set
	 * is not empty, it colors the nodes into the visualization.
	 *
	 * @param G             graph to visualize.
	...
	/**
	 * This method allows a fast visualization of a graph. If the selectednodes and
	 * selectedNodes2 sets are not empty, it colors the two groups of nodes into the
	 * visualization.
	 *
	 * @param G              graph to visualize.
	...
	/**
	 * Returns a 2-dimension array corresponding to the B-Matrix of the graph. See
	 * Bagrow and al. 2008 "Portraits of complex networks".
	 *
	 * @param G graph
