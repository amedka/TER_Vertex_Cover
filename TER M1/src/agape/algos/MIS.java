...
/**
 * This class computes the maximum independ set of a graph (directed or
 * undirected). This problem consists in finding the maximum set of vertices
 * such that two vertices of the computed set are not neighbors in the original
 * graph. Several algorithms have been implemented.
 *
 * @author V. Levorato
 * @author J.-F. Lalande
 * @author P. Berthome
...
public class MIS<V, E> extends Algorithms<V, E> {
	...
	/**
	 * This method finds a maximal independent set in a graph using a trivial greedy
	 * algorithm. The complexity of this algorithm is polynomial (O(n+m)).
	 *
		...
		while (!Gp.getVertices().isEmpty()) {
			...
			if (x != null) {
				...
				if (Gp.getNeighbors(x) != null) {
					/*
					 * N.addAll(Gp.getNeighbors(x)); for(V y : N ) M.add( y);
					 */
	 ...
	 * force algorithm that examines every vertex subset and checks whether it is an
	 * independent set, which time complexity is O(n^2.2^n).
	 *
	...
	/**
	 * This method returns a maximum independent set of G based on the Moon and
	 * Moser work ("On cliques in graphs" 1965). The algorithm solves the problem in
	 * time O(1.4423^n).
	 *
	 * @param g Graph in which Maximum Independent sets have to be found
	 * @return A maximum independent set of G
	 ...
	 * work ("A Measure & Conquer Approach for the Analysis of Exact Algorithms"
	 * 2009). The algorithm solves the problem in time O(1.2201^n).
	 *
	 * @param g graph
	...
	public Set<V> maximuRmIndependentSetFominGrandoniKratsch(Graph<V, E> g) {
		...
		if (deg2) {
			...
			Set<V> F = this.maximuRmIndependentSetFominGrandoniKratsch(
					MIS.getFoldingGraph(g, x, vftable, this.vertexFactory, this.edgeFactory));
		...
		Set<V> S1 = this.maximuRmIndependentSetFominGrandoniKratsch(G_v_Mv);
		Set<V> S2 = this.maximuRmIndependentSetFominGrandoniKratsch(G_Nv);

		S2.add(d);

		if (S1.size() > S2.size()) { // if(tracker!=null) tracker.increase("M branch("+M.size()+")"); //tracker
										// operation
			return S1;
		} else { // if(tracker!=null) tracker.increase("Nv branch"); //tracker operation
			return S2;
		}
	 ...
	 * The fold parameter permits to memorize folding operation and to unfold
	 * vertices if needed.
	 *
	 * @param G    Graph where to fold v.
	 * @param v    vertex used to fold.
	 * @param fold hashtable where keys are merged vertices and where values are
	 *             these same vertices unmerged.
	 * @return a Graph where v has been folded.
	 */
	protected static <V, E> Graph<V, E> getFoldingGraph(Graph<V, E> G, V v, HashMap<V, Set<V>> fold,
	...
	/**
	 * This method returns the vertices mirrors of v in Graph G. (Fomin, Grandoni
	 * and Kratsch, 2009).
	 *
	 * @param G Graph
	 * @param v vertex
