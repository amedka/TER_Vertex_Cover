...
public class RandGenerator<V,E> {
    ...
    /**
     * This method generates Eppstein power law graph. Generator from JUNG library is used in this method.
     * @param graphFactory graph factory
     * @param vertexFactory vertex factory
     * @param edgeFactory edge factory
     * @param n number of vertices 
     * @param e number of edges
     * @param i the number of iterations to use: the larger the value the better the graph's degree distribution will approximate a power-law
    ...
    /**
     * This method generates Barabasi-Albert power law graph. Generator from JUNG library is used in this method.
     * @param graphFactory graph factory
     * @param vertexFactory vertex factory
     * @param edgeFactory edge factory
     * @param n amount of nodes
     * @param e the number of edges that should be attached from the new vertex to pre-existing vertices at each time step
     ...
     * @return Barabasi-Albert power law graph with n+steps vertices and steps * e edges.
    ...
    {
        if (e > n) throw new Exception("Impossible to attach the " + e + " new edges to " + n + " vertices.");
     ...
     * @param p Manhattan distance of the all nodes to connect to the neighbors of a node v
     * @param r A constant to manage the probability of connecting a node v to a long distance node
    ...
    public static<V,E> Graph<V,E> generateWattsStrogatzSWGraph(Factory<Graph<V,E>> graphFactory,
        ...
        if ((p<0) || (p>1)) throw new Exception("Probabilities must be between 0 and 1");

                Graph<V,E> G =    
                    NRandGenerator.generateRegularRing(graphFactory, vertexFactory,
                            edgeFactory, n, k);
                ...
                for(V v : vertices)
                {
                    ...
                    for(V nv :Nv)
                        if(Math.random()<p)
                        {
                            ...
                            if(!Operations.isEdge(G, v, new_nv) &&
                                    new_nv!=null) G.addEdge(edgeFactory.create(),
                                            v, new_nv); }
                }
         ...
         do
         {
            ...
            while(!S.isEmpty())
            {
                ...
                while(!stop)
                {
                    ...
                    {
                        Pair<V,V> p=itr.next();
                        double pp=((double)(d-G.degree(p.getKey()))*(d-G.degree(p.getValue())))/(double)S.size();
                        if(Math.random()<pp)
                            { pchoosen=p; stop=true; }
                    }
                }
